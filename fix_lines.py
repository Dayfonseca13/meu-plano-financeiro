...
def replace_block(start_match, end_match, new_content):
    global lines
    start_idx = -1
    end_idx = -1
    ...
    for i in range(start_idx, len(lines)):
        if end_match in l and l.startswith("});"):
            pass
            # wait, it's easier to just match next route
    
    # We already know the line numbers roughly, but we can do string replace
    # Just read file as string and replace.
...
health_old = re.search(r"app\.get\('/api/health', async \(req: Request, res: Response\) => \{.*?\}\);", content, flags=re.DOTALL)
if health_old:
    content = content.replace(health_old.group(0), """app.get('/api/health', (req: Request, res: Response) => {
  res.json({
    ok: true,
    server: true,
...
register_old = re.search(r"app\.post\('/api/users/register', async \(req: Request, res: Response\) => \{.*?\}\);", content, flags=re.DOTALL)
if register_old:
    content = content.replace(register_old.group(0), """app.post('/api/users/register', async (req: Request, res: Response) => {
  try {
...
login_old = re.search(r"app\.post\('/api/users/login', async \(req: Request, res: Response\) => \{.*?\}\);", content, flags=re.DOTALL)
if login_old:
    content = content.replace(login_old.group(0), """app.post('/api/users/login', async (req: Request, res: Response) => {
  try {
